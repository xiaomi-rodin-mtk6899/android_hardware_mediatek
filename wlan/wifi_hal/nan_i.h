...
#define FW_MAC_ADDR_TO_CHAR_ARRAY(fw_mac_addr, mac_addr)              \
    do {                                                              \
...
#define CHAR_ARRAY_TO_MAC_ADDR(mac_addr, fw_mac_addr)                                  \
    do {                                                                               \
        (fw_mac_addr).mac_addr31to0 = ((mac_addr)[0] | ((mac_addr)[1] << 8) |          \
                                       ((mac_addr)[2] << 16) | ((mac_addr)[3] << 24)); \
        (fw_mac_addr).mac_addr47to32 = ((mac_addr)[4] | ((mac_addr)[5] << 8));         \
    } while (0)
...
#define NAN_MAX_CONFIGURATION_REQ_SIZE                                                         \
    (sizeof(NanMsgHeader) + SIZEOF_TLV_HDR + sizeof(u8) /* SID Beacon    */ + SIZEOF_TLV_HDR + \
     sizeof(u8) /* Random Time   */ + SIZEOF_TLV_HDR + sizeof(u8) /* Master Pref   */          \
    )
...
#define NAN_MAX_ENABLE_REQ_SIZE                                                                 \
    (sizeof(NanMsgHeader) + SIZEOF_TLV_HDR + sizeof(u16) /* Cluster Low   */ + SIZEOF_TLV_HDR + \
     sizeof(u16) /* Cluster High  */ + SIZEOF_TLV_HDR + sizeof(u8) /* Master Pref   */          \
    )
...
#define NAN_MAX_TCA_IND_SIZE                                           \
    (sizeof(NanMsgHeader) + sizeof(NanTcaIndParams) + SIZEOF_TLV_HDR + \
     sizeof(u16) /* Cluster Size */                                    \
    )
